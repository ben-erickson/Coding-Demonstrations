...
for x in range(len(gradebook[0])):
    assignment_ave = 0
    for y in range(len(gradebook)):
        assignment_ave += gradebook[y][x]
    assignment_ave /= len(gradebook)
    print("Assignment " + str(x+1) + ": %.2f" % assignment_ave)
...
for x in range(len(gradebook)):
    stu_ave = 0
    for y in gradebook[x]:
        stu_ave += y
    stu_ave /= len(gradebook[x])
    print("Student " + str(x+1) + ": %.2f" % stu_ave)
